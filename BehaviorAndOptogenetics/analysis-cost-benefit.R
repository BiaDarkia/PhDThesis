...
tidy_data <- read.table("./tidy_data_dreadds.csv", header=TRUE, sep = ",")
arch <- read.table("./dreadds.csv", header=TRUE, sep = ",")
...
for (c in c(1, 2, 3)) {
        ...
        anova_rt <- ezANOVA(data = data_cond, dv = rt_diff, wid = subject, within = opto, 
            between = arch, type = 2, return_aov = TRUE)
        ...
        for (a in c(1, 0)) {
                ...
                levenes_rt = append(levenes_rt, list(leveneTest(rt_diff~as.factor(opto), data_arch)))
                shapiro_rt = append(shapiro_rt, list(shapiro.test(data_arch$rt_diff[data_arch$opto=='0']-data_arch$rt_diff[data_arch$opto=='1'])))
                
                ttests_rt_within = append(ttests_rt_within, list(t.test(data_arch$rt_diff[data_arch$opto=='0'], 
...
axis(side = 1, at = 1:2, labels=c("Light OFF", "Light ON"), 
    lwd=5, mgp=c(3, 2, 0), cex.axis=2.2, font=2)
axis(side=2, at=seq(-4,4,1), labels=labels_rt, las=1, lwd=5, cex.axis=2.2, font=2)
...
setnames(data_noopto, 5:12, paste0(names(data_noopto)[5:12], '_noopto'))
setnames(data_opto, 5:12, paste0(names(data_opto)[5:12], '_opto'))
...
for (c in c(1, 2, 3)) {
    ...
    levenes_rt_diff = append(levenes_rt_diff, list(leveneTest(diff_rt_diff~as.factor(arch), data_cond)))
    ks_rt_diff = append(ks_rt_diff, list(ks.test(data_cond$diff_rt_diff[data_cond$arch=='0'], data_cond$diff_rt_diff[data_cond$arch=='1'])))
    ttests_rt_diff = append(ttests_rt_diff, list(t.test(data_cond$diff_rt_diff[data_cond$arch=='1'], 
        data_cond$diff_rt_diff[data_cond$arch=='0'], paired=FALSE)))
    mw_rt_diff = append(mw_rt_diff, list(wilcox.test(data_cond$diff_rt_diff[data_cond$arch=='1'], 
        data_cond$diff_rt_diff[data_cond$arch=='0'], paired=FALSE)))
    ...
    p_rt = mw_rt_diff[[length(mw_rt_diff)]]$p.value
    eff_rt = abs(qnorm(p_rt/2)/sqrt(nrow(data_cond)))
    
    diff_tests_output[nrow(diff_tests_output) + 1,] = list(c, 'rt', 
        levenes_rt_diff[[length(levenes_rt_diff)]]$`Pr(>F)`, ks_rt_diff[[length(ks_rt_diff)]]$p.value, p_rt, eff_rt)
}
